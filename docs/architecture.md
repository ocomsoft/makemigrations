...
> compiler. For migrations generated by `makemigrations makemigrations`
...
The `Graph` built from the `Registry` is the authoritative record of migration history and schema state. There is no separate snapshot file in the Go workflow. `makemigrations makemigrations` reads the current state by calling `interp.LoadRegistry` + `BuildGraph` + `ToDAGOutput` directly — no external process, no JSON serialisation round-trip.
...
1. **No direct SQL execution by the generator** — `makemigrations makemigrations` only writes `.go` files. SQL is only executed when the developer explicitly runs `makemigrations migrate up` (or the optional standalone binary).
