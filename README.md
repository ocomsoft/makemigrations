...
> `morphic migrate` does **not** invoke `go build`. Instead it loads
> each `.go` file with [yaegi](https://github.com/traefik/yaegi), an
> embedded Go interpreter, and runs the migrations in the morphic
...
> differently. For migrations generated by `morphic generate`
