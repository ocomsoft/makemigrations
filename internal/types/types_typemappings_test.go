...
`
	...
	if got := s.TypeMappings[DatabasePostgreSQL]["float"]; got != "NUMERIC(9,2)" {
		t.Errorf("postgresql float = %q, want %q", got, "NUMERIC(9,2)")
	}
	if got := s.TypeMappings[DatabasePostgreSQL]["boolean"]; got != "SMALLINT" {
		t.Errorf("postgresql boolean = %q, want %q", got, "SMALLINT")
	}
	if got := s.TypeMappings[DatabaseMySQL]["float"]; got != "DOUBLE" {
		t.Errorf("mysql float = %q, want %q", got, "DOUBLE")
	}
	if s.TypeMappings[DatabaseSQLite] != nil {
		t.Errorf("sqlite should be nil, got %v", s.TypeMappings[DatabaseSQLite])
	}
}

func TestTypeMappings_ForProvider(t *testing.T) {
	tm := TypeMappings{
		DatabasePostgreSQL: map[string]string{"float": "NUMERIC"},
		DatabaseMySQL:      map[string]string{"float": "DOUBLE"},
	}
