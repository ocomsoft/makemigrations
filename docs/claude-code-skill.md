...
3. Generate migration code
   └─ Run: makemigrations generate --name "description"
...
3. **RunSQL is last resort**: Only used for data migrations, complex constraints, or database-specific features that can't be expressed in the schema format. Claude uses `makemigrations generate empty --name "description"` to create the shell file.
...
- **All commands**: init, makemigrations, migrate (up/down/status/showsql/dag), empty, db2schema, struct2schema, dump-data (via `makemigrations` CLI)
...
3. Runs: makemigrations generate --name "add_profiles"
