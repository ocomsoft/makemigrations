...
```
makemigrations generate --name "add_phone"
...
## The `makemigrations generate` Flow
...
makemigrations generate --name "add_user_phone"
...
Implement the `dag` subcommand with `--format json` and `--format ascii` output. ASCII renderer with tree-drawing characters. Wire the `makemigrations generate` command to build the binary, query it, and parse the JSON output.
