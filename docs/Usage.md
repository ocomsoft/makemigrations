...
```bash
makemigrations schema-to-sql --database postgresql
```
...
```bash
makemigrations generate --name "add_orders"
```
...
```bash
makemigrations generate --name "add_user_phone_and_last_login"
makemigrations migrate up
```
...
```bash
makemigrations generate --name "add_user_role_status_index"
makemigrations migrate up
```
...
```bash
makemigrations generate --name "expand_user_status_length"
```
...
```bash
makemigrations generate --name "remove_order_notes"
```
...
```bash
makemigrations generate --name "remove_product_categories"
```
...
| `makemigrations generate` | Generate a migration from schema changes |
| `makemigrations generate --dry-run` | Preview migration source without writing a file |
| `makemigrations generate --check` | CI mode: exit 1 if migrations are needed |
| `makemigrations generate --merge` | Generate a merge migration for concurrent branches |
| `makemigrations current-state` | Show reconstructed schema from migration DAG as YAML |
| `makemigrations schema-to-sql` | Show full CREATE TABLE SQL from the YAML schema |
| `makemigrations schema-to-sql --verbose` | Include processing detail in the output |
